...
def get_acs_links(services, service_filter):
    ...
    for service in services:
        if service.info:
            ...
            need_lb = compose_labels.get("aliyun.proxy.required", "false") == "true"
            has_virtual_host = compose_labels.get("aliyun.proxy.VIRTUAL_HOST", None)
            compose_project = service.info.get("project")
            compose_service = service.info.get("name")
            linked_compose_services.append("%s_%s" % (compose_project, compose_service))
            service_name = "%s_%s" % (compose_project, compose_service)
            for _container in service.containers:
                if (need_lb or has_virtual_host) and service_filter(compose_project, compose_service):
    ...
    return links, ["%s" % service for service in linked_compose_services]
